...
const filterElList = document.getElementsByClassName('filter-nav__toggle--first');
const filterEl = filterElList[0];

/******** HIDING PRODUCTS FROM PLPs BY BADGE MJ 01-25-2022 GH-982 *******/
...
// set which badge to show according to URL
for (let i=0; i<badgesToHide.length; i++){
  var re = new RegExp(badgesToHide[i].url);
  if(re.test(currentLocation)){ // if the current location matches the current badgesToHide.url
    badgeToShow = badgesToHide[i].badge; // set the badgeToShow to the current badgesToHide.badge
  }
}
...
(function() {
  ...
  ProductGridItem.prototype.compileTemplate = function(data) {
    ...
    fetch(`/products/${data.handle}?view=json`)
      .then((blob) => blob.json())
      .then((response) => {

        const contentEl = document.querySelector(`[data-tile-content="${data.id}"]`);
        const bottomLineEl = document.querySelector(`[data-product-id="${data.id}"]`);
        const badges = document.querySelector(`[data-badges-for="${data.id}"]`);
        const price = document.querySelector(`[data-price-for="${data.id}"]`);

        price.classList.remove('hidden');
        if (response.comparePrice) {
          price.innerHTML = `
            <span class="price price--sale">$${(response.price / 100.0).toFixed(2)}</span>
            <span class="price price--old">$${(response.comparePrice / 100.0).toFixed(2)}</span>
          `;
        } else {
          price.innerHTML = `$${(response.selectedVariant.price / 100.0).toFixed(2)}`;
        }

        contentEl.classList.remove('hidden');
        if (response.metafields.miniDescription) {
          contentEl.innerHTML = response.metafields.miniDescription;
        } else {
          const firstParIndex = response.description.indexOf('</p>');
          contentEl.innerHTML = response.description.substring(0, firstParIndex + 4);
        }

        if(response.metafields.yotpo.reviews_count == 0) {
          bottomLineEl.innerHTML = '<div class="yotpo-display-wrapper" aria-hidden="true" style="visibility: hidden;"> <div class="standalone-bottomline" tabindex="-1"> <div class="yotpo-bottomline pull-left star-clickable" tabindex="0"> <span class="yotpo-stars"> <span class="yotpo-icon yotpo-icon-empty-star rating-star pull-left"></span><span class="yotpo-icon yotpo-icon-empty-star rating-star pull-left"></span><span class="yotpo-icon yotpo-icon-empty-star rating-star pull-left"></span><span class="yotpo-icon yotpo-icon-empty-star rating-star pull-left"></span><span class="yotpo-icon yotpo-icon-empty-star rating-star pull-left"></span><span class="sr-only">0.0 star rating</span> </span> <a class="text-m write-review-btn-hidden">Write a review</a> <div class="yotpo-clr"></div> </div> <div class="yotpo-clr"></div> </div> <div class="yotpo-clr"></div> </div>'
        } else {
          bottomLineEl.innerHTML = `<div class="yotpo bottomLine" data-product-id="${data.id}"> </div>`
        }

        if (response.tags.includes('badge-out-of-stock') ||
            response.tags.includes('badge-discontinued') ||
            response.available === false) {
          const quickshopButton = document.querySelector(`[data-quickbuy-trigger="${response.handle}"]`);
          quickshopButton.classList.add('hidden');
        }
        ...
        sortedTags.forEach((tag) => {
          if (tag.includes('badge-')) {
            ...
            badges.innerHTML += `<div class="badge badge--${badgeText}">${badgeText}</div>`;
          }
        });
        if (!response.available) {
          badges.innerHTML += `<div class="badge badge--out-of-stock">Out Of Stock</div>`;
        }
      });

    // Custom filters
    itemHtml = itemHtml.replace(/{{itemFeatureImage}}/g, data.images[1]);
    itemHtml = itemHtml.replace(/{{itemID}}/g, data.id);
    // itemHtml = itemHtml.replace(/{{itemRPrice}}/g, data.price_min);
    itemHtml = itemHtml.replace(/{{itemURL}}/g, `/products/${data.handle}`);
    // itemHtml = itemHtml.replace(/{{itemDesecription}}/g, `/products/${data.description}`);

    // const productHoverImage = data.images[2].replace('.jpg', '_220x.jpg').replace('.png', '_220x.png');
    // const productHoverImage = Utils.optimizeImage(data.images.length > 1 ? data.images[1] : data.images[0], '220x');
    const hoverImage = Object.keys(data.images).length > 1 ? data.images[2] : data.images[1];
    const productHoverImage = hoverImage !== null ? hoverImage.replace('.jpg', '_220x.jpg').replace('.png', '_220x.png') : data.images[1];

    itemHtml = itemHtml.replace(/{{itemAltImage}}/g, productHoverImage);
  ...
  ProductSorting.prototype.render = function() {
    ...
    jQ('.boost-pfs-filter-custom-sorting button').text(labelSort);
  }
  ...
  ProductSorting.prototype.bindEvents = function() {
    ...
    jQ(".boost-pfs-filter-custom-sorting > button").click(function() {
      if (!jQ('.boost-pfs-filter-filter-dropdown').is(':animated')) {
        ...
        jQ('.select--button').toggleClass('open');
      }
    });
  ...
  Filter.prototype.afterRender = function(data) {
    if (!data) data = this.data;
    var pageResultTotal = data.total_page;

    // var totalProduct = data.total_product + '<span> ' + boostPFSThemeConfig.label.items_with_count_other + '</span>';

    var totalProduct = '<span>Products (' + data.total_product + ')</span>';
    var totalPageHTML = '<span>Pages (' + pageResultTotal + ')</span>';
    var totalNumberproduct = data.total_product + ' Products' ;
    if (data.total_product == 1) {
      totalProduct = '<span>Products (' + data.total_product + ')</span>';
    }
    jQ('.boost-pfs-filter-total-product').html(totalProduct);
    jQ('.boost-pfs-filter-total-page').html(totalPageHTML);
    jQ('.product-controls__count').text(totalNumberproduct);
  ...
  FilterApi.beforeCall = function() {
    if (Utils.isSearchPage() && !Globals.queryParams.event_type) {
      Globals.queryParams.sort = 'best-selling';
    }
/******** HIDING PRODUCTS FROM PLPs BY BADGE MJ 01-25-2022 GH-982 *******/
    if(badgeToShow !== 'badge-final-sale'){ // not on /outlet, hide products
      Globals.queryParams.h_options = 'badge-final-sale'; // sets the query param to filter out products with badge
    }
    // tell the API to not use cache
    Globals.queryParams.check_cache = false;
/******** END GH-982 */
  }
